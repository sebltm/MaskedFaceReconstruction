...
def train_gen(gen: G.ClassicGenerator, disc: D.SiameseCaps,
    ...
    file_writer = tf.summary.create_file_writer(constants.imdir)
...
def train(disc_1: D.SiameseCaps, disc_loss_fn_1: tf.keras.losses.Loss, disc_optimizer_1: tf.keras.optimizers.Optimizer,
    ...
    for epoch in range(123, epochs):
        ...
        if True or min_val is None or total_metric < min_val:
...
def train_step(disc_1: D.SiameseCaps, disc_loss_fn_1: tf.keras.losses.Loss, disc_optimizer_1: tf.keras.optimizers.Optimizer,
    ...
    with tf.GradientTape(watch_accessed_variables=False) as triplet_tape:
        triplet_tape.watch(disc_1.variables)

        # train the discriminator in real samples
        triplet_input = tf.stack((anchor, positive_diff, negative), axis=1)
        triplet_output_real = disc_1(triplet_input, training=True)

        # train the discriminator on fake sample
        triplet_input = tf.stack((anchor, positive_diff, generated_image), axis=1)
        triplet_output_fake = disc_1(triplet_input, training=True)

        if file_writer is not None and step % 100 == 0:
            with file_writer.as_default():
                images = np.reshape(triplet_input.numpy(), (-1, constants.IMAGE_SIZE, constants.IMAGE_SIZE, 3))
                tf.summary.image(f"disc_data_epoch_{epoch}", images, step=step)

        # follow down gradient for discriminator
        triplet_loss_fake = disc_loss_fn_1(None, triplet_output_fake)
        triplet_loss_real = disc_loss_fn_1(None, triplet_output_real)
        triplet_total_loss = tf.keras.backend.relu(triplet_loss_real + triplet_loss_fake)

    disc_caps_metric.update_state(triplet_total_loss)

    triplet_grads = triplet_tape.gradient(triplet_total_loss, disc_1.trainable_variables)
    disc_optimizer_1.apply_gradients(zip(triplet_grads, disc_1.trainable_variables))

    ### TRAIN DISCRIMINATOR AGENT 2 ###
    with tf.GradientTape(watch_accessed_variables=False) as began_tape:
        began_tape.watch(disc_2.variables)
    ...
    disc_began_metric.update_state(began_loss)

    began_grads = began_tape.gradient(began_loss, disc_2.trainable_variables)
    disc_optimizer_2.apply_gradients(zip(began_grads, disc_2.trainable_variables))
...
if __name__ == "__main__":
    ...
    disc_loss_fn_1 = D.SiameseCaps.TripletLoss(scale=10e13)
